...
# Fix Windows charmap error by overriding print to strip emojis
built_in_print = print
def safe_print(*args, **kwargs):
    safe_args = [arg.encode('ascii', 'ignore').decode('ascii') if isinstance(arg, str) else arg for arg in args]
    built_in_print(*safe_args, **kwargs)
print = safe_print
    ...
    try:
        ...
        if league is not None:
            ...
            if manager_chip_history and league:
                manager_id_to_name = {m['entry']: m['player_name'] for m in standings_data}
                pdf.add_chip_matrix(manager_chip_history, manager_id_to_name, gw, "Chip Usage Matrix - All Gameweeks (All Managers)")
            else:
        ...
        dashboard_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "dashboard", "public")
        os.makedirs(dashboard_dir, exist_ok=True)
        pdf_path = os.path.join(dashboard_dir, "gw_report.pdf")
        pdf.output(pdf_path)
        print(f"✅ Comprehensive PDF report saved to dashboard public dir: {pdf_path}")
